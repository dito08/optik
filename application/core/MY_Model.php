...
class CMS_Model extends CI_Model
{
    ...
    {
        $query = $this->db->query("SELECT user_id, user_name, first_name, email FROM ".cms_table_name('main_user')." WHERE
     ...
     */
    public function cms_do_register($user_name, $email, $first_name, $last_name, $password)
    {
        ...
        $this->db->insert(cms_table_name('main_user_address'), $data2);
        // send activation code if needed
        if ($need_activation) {
            $this->cms_generate_activation_code($email, TRUE, 'SIGNUP');
        }
     ...
     */
    public function cms_do_change_profile($user_name, $email, $first_name, $last_name, $password = NULL)
    {
        $data = array(
            "user_name" => $user_name,
            "email" => $email,
     ...
     * @desc    change current profile (user_name, email, real_name and password)
     */
    public function cms_do_change_address($user_name, $first_name, $last_name, $company=NULL, $handphone=NULL, $telephone=NULL, $address=NULL, $city=NULL, $state=NULL, $postal_code=NULL, $country=NULL)
    {
        $data = array(
            "first_name"    => $first_name,
            "last_name"     => $last_name,
        ...
        $this->db->update(cms_table_name('main_user_address'), $data, $where);
    }
    ...
    public function cms_generate_activation_code($identity, $send_mail = FALSE, $reason = 'FORGOT')
    {
        ...
        // generate query
        $query = $this->db->query("SELECT user_name, first_name, last_name, user_id, email FROM ".cms_table_name('main_user')." WHERE
                    (user_name = '" . addslashes($identity) . "' OR email = '" . addslashes($identity) . "') AND
                    $where_active");
        if ($query->num_rows() > 0) {
            $row              = $query->row();
            ...
            if ($send_mail) {
                ...
                $email_message = str_replace('{{ user_real_name }}', $first_name, $email_message);
    ...
    public function cms_third_party_login($provider)
    {
        ...
        $query = $this->db->select('user_id, user_name, email, first_name')->from(cms_table_name('main_user'))->where('user_id', $user_id)->get();
        if ($query->num_rows() > 0) {
            $row = $query->row();
            $this->cms_user_id($row->user_id);
            $this->cms_user_name($row->user_name);
            $this->cms_user_real_name($row->first_name);
    ...
    public function check_order_exist($user_id)
    {
        $query = $this->db->select('*')
            ->from(cms_table_name('commerce_order'))
    ...
    public function get_unpayment_order($user_id)
    {
    ...
    public function cms_ecommerce_order($identity)
    {
        $query = $this->db->select('*')
            ->from(cms_table_name('commerce_order'))
    ...
    public function cms_do_save_order($order_id,$data,$detail)
    {
        $query = $this->db->select($order_id)
            ->from($this->cms_complete_table_name('commerce_order'))
        ...
        if($num_row>0){
            ...
            $this->db->insert(cms_table_name('commerce_order'), $data);

            foreach ($detail as $detail_value) {
                $this->db->insert(cms_table_name('commerce_order_detail'), $detail_value);
            }
            return TRUE;
        }
    }

    public function do_confirm_payment($order_no,$user_id,$data)
    {
        $query = $this->db->select($order_id)
            ->from($this->cms_complete_table_name('commerce_order'))
        ...
        if($num_row>0){
            ...
            $this->db->limit(1);
            $this->db->update(cms_table_name('commerce_order'), $data);
            return TRUE;
        }else{
            return FALSE;
        }
    ...
    public function cms_ecommerce_order_detail($identity)
    {
        $data = array();
        $query    = $this->db->select('A.order_no, A.product_id, A.qty, A.subtotal, B.id, B.product_name, B.brand, B.type, C.size')
            ->from(cms_table_name('commerce_order_detail A'))
            ->join(cms_table_name('commerce_product B'), 'B.id = A.product_id')
            ->join(cms_table_name('commerce_size C'), 'C.id = A.size')
            ->where('A.order_no', $identity)
            ->get();
        $result = $query->result();
        ...
        foreach($query->result() as $row){
            $data[] = array(
                ...
                "brand"        => $row->brand,
                "type"         => $row->type,
                "image_url"    => $this->cms_get_photos_shop($row->id),
                "size"         => $row->size,
                "qty"          => $row->qty,
    ...
    public function cms_ecommerce_have_buy($identity=NULL,$product_id=NULL)
    {
        $data = array();
        ...
        if($identity != NULL)
        {
            ...
            $query    = $this->db->select('A.order_no,A.product_id, A.qty, A.subtotal, B.id, B.product_name, B.brand, B.type, C.size')
                    ...
                    ->join(cms_table_name('commerce_product B'), 'B.id = A.product_id')
                    ->join(cms_table_name('commerce_size C'), 'C.id = A.size')
                    ->join(cms_table_name('commerce_order D'), 'D.order_no = A.order_no')
                    ->where('D.user_id', $identity)
                    ->limit('4')
                    ->get();
           ...
           $SQL = "
                    SELECT A.*
                    FROM ".cms_table_name('commerce_product A')."
                    JOIN ".cms_table_name('commerce_product_category B')." ON B.product_id = A.id
                    WHERE B.category_id IN (SELECT category_id FROM ".cms_table_name('commerce_product_category')." WHERE product_id = '$identity') AND B.product_id != '$identity' LIMIT 4";
        ...
        else
        {
            ...
            $SQL = "
                SELECT id, product_id, product_name, brand, type, price, availibility, info, category
                FROM ".cms_table_name('commerce_product')."
                WHERE id = '$product_id'";        
    ...
    public function cms_ecommerce_order_info($identity)
    {
        $query    = $this->db->select('*')
            ->from(cms_table_name('commerce_order'))
            ->where('order_no', $identity)
            ->get();
        $num_rows = $query->row();
        return $num_rows;
    }

    public function cms_ecommerce_wishlist($identity)
    {
        $data = array();
        $query    = $this->db->select('A.*,B.price,B.info,B.brand,B.type,B.product_name')
            ->from(cms_table_name('commerce_wishlist A'))
            ->join(cms_table_name('commerce_product B'), 'B.id = A.product_id')
            ->where('A.user_id', $identity)
            ->get();
        $num_rows = $query->result();
        ...
        foreach($query->result() as $row){
            $data[] = array(
                ...
                "brand"        => $row->brand,
                "type"         => $row->type,
                "image_url"    => $this->cms_get_photos_shop($row->product_id),
                "qty"          => $row->qty,
                "info"         => $row->info,
    ...
    public function cms_ecommerce_add_wishlist($product_id,$user_id,$qty)
    {
        $query = $this->db->select('*')
            ->from(cms_table_name('commerce_wishlist'))
            ->where('user_id', $user_id)
        ...
        if($num_rows>0){
            ...
            $sql = $this->db->insert(cms_table_name('commerce_wishlist'), $data);
            return TRUE;
        }      
    }

    public function cms_ecommerce_remove_wishlist($wishlist_id)
    {
        $where = array(
            "id" => $wishlist_id
        );
        $query = $this->db->delete(cms_table_name('commerce_wishlist'), $where);

        return TRUE;
    }

    public function cms_ecommerce_product($identity,$limit)
    {
        $data = array();
        ...
        if($identity == "all"){
            ...
            if($limit = NULL){
                $SQL = "
                    ...
                    FROM ".cms_table_name('commerce_product A')."
                    JOIN ".cms_table_name('commerce_product_category B')." ON B.product_id = A.id
                    WHERE B.category_id = '$identity' LIMIT '$limit'";
            }else{
                $SQL = "
                    SELECT A.*
                    FROM ".cms_table_name('commerce_product A')."
                    JOIN ".cms_table_name('commerce_product_category B')." ON B.product_id = A.id
                    WHERE B.category_id = '$identity'" ;
            }
        }
        ...
        foreach($query->result() as $row){
            $data[] = array(
                    ...
                    "photos" => $this->cms_get_photos_shop($row->id)
            );
        }
        return $data;
    }

    public function cms_get_size($id){
        $SQL = "
            SELECT B.*
            FROM ".cms_table_name('commerce_product_size A')."
            LEFT JOIN ".cms_table_name('commerce_size B')." ON B.id = A.size_id
    ...
    public function cms_get_photos($id){
        $query = $this->db->select('')
    ...
    public function cms_get_photos_shop($id){
    ...
    public function cms_ecommerce_product_detail($id){
        $data = array();

        $SQL = "
            SELECT id, product_id, product_name, brand, type, weight, show_m2, price, old_price, tax, availibility, info, category
            FROM ".cms_table_name('commerce_product')."
            WHERE id = '$id'";
        ...
        foreach($query->result() as $row){
            $data[] = array(
                ...
                "info"         => $row->info,
                "category"     => $row->category,
                "photos"       => $this->cms_get_photos($row->id),
                "size"         => $this->cms_get_size($row->id)
            );
        }
        return $data;
    }

    public function cms_ecommerce_product_cart($id){
        $data = array();

        $SQL = "
            SELECT id, product_id, product_name, brand, type, weight, price, availibility, info, category
            FROM ".cms_table_name('commerce_product')."
            WHERE id = '$id'";
    ...
    public function cms_ecommerce_gallery($identity)
    {
        $data = array();

        if($identity == "all"){
            $SQL = "
                SELECT B.id, B.product_id, A.product_name, A.brand, A.type, B.place, B.category, B.image_url
                FROM ".cms_table_name('gallery_product B')."
                JOIN ".cms_table_name('commerce_product A')." ON B.product_id = A.id";
        }else{
            $SQL = "
                SELECT B.id, B.product_id, A.product_name, A.brand, A.type, B.place, B.category, B.image_url
                FROM ".cms_table_name('gallery_product B')."
                JOIN ".cms_table_name('commerce_product A')." ON B.product_id = A.id
                WHERE B.category = '$identity'";
        }
    ...
    public function cms_ecommerce_gallery_info($id)
    {
        $SQL = "
                SELECT B.id, B.product_id, A.product_name, A.brand, A.type, B.place, B.category, B.image_url
                FROM ".cms_table_name('gallery_product B')."
    ...
    public function search($terms)
    {
        ...
        /*$sql = "SELECT *
                FROM ".cms_table_name('commerce_product')."
                WHERE info LIKE '%$terms%' OR product_category LIKE '%$terms%'";*/
        $sql = "SELECT A.*, B.category_name, C.*
                FROM cms_commerce_product_category A
                LEFT JOIN cms_commerce_category B ON B.id = A.category_id
                LEFT JOIN cms_commerce_product C ON C.id = A.product_id
                WHERE C.info LIKE '%$terms%' OR B.category_name LIKE '%$terms%'
                GROUP BY C.product_name, brand, type";
        $query = $this->db->query($sql, array($terms, $terms));
        return $query->result();
    }

    public function get_available_product_category()
    {
        $result = array();
        $query = $this->db->select('id, category_name')
            ->from(cms_table_name('commerce_category'))
            ->get();
        foreach($query->result() as $row){
            $result[] = array(
                "id"   => $row->id,
    ...
    public function cms_get_testimonial($limit=NULL)
    {
        $data = array();
        ...
        if(!$limit){
            $SQL = "
                ...
                FROM ".cms_table_name('commerce_testimonial A')."
    ...
    public function find_city($keyword)
    {
        $this->db->like('kota',$keyword,'after');
        $this->db->or_like('kecamatan',$keyword,'after');
        $query=$this->db->get(cms_table_name('shipping_jne'));
        return $query->result_array();
    }

    public function cms_ecommerce_shipping($city)
    {
        $query = $this->db->select('harga')
            ->from(cms_table_name('shipping_jne'))
            ->where('noongkir', $city)
            ->get();
        $row = $query->row();
        return $row;
    ...
    {
        $todaysDate = date("Y-m-d");
        $query = $this->db->select('A.*, C.first_name, C.last_name')
            ->from(cms_table_name('commerce_order A'))
            ->join(cms_table_name('main_user B'),'B.user_id = A.user_id','left')
            ->join(cms_table_name('main_user_address C'),'C.user_name = B.user_name','left')
            ->where('date', $todaysDate)
            ->get();
        $result = $query->result();
